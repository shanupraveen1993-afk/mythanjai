...
export default function ListingCard({ listing, isPreview }: { listing: ListingItem; isPreview?: boolean }) {
  ...
  return (
    <>
      <div className="bg-white rounded-none sm:rounded-2xl p-3.5 sm:p-4 flex flex-col justify-between shadow-none sm:shadow-2xs border-b sm:border border-slate-200/80 sm:border-slate-200/90 relative font-sans h-full w-full">
              ...
              <>
                {listing.show_phone === false || !listing.phone ? (
                  <button
                    ...
                    className={`w-[110px] sm:w-[128px] shrink-0 bg-[#1F244A] hover:bg-[#151936] text-white font-heading font-black text-xs sm:text-sm py-2.5 px-3 rounded-xl flex items-center justify-center gap-1.5 min-h-[46px] shadow-md cursor-pointer transition-colors whitespace-nowrap ${isPreview ? "opacity-60 pointer-events-none" : ""}`}
                  >
