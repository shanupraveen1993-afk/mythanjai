...
export function RobotHero({
  ...
  return (
    <section
      ref={containerRef}
      className="relative w-full h-[calc(100dvh-60px)] md:h-auto md:min-h-[calc(100vh-70px)] flex flex-col justify-between items-center bg-white text-slate-800 pt-2 pb-4 sm:py-5 px-3 sm:px-4 overflow-hidden select-none"
    >
      ...
      <div className="relative z-10 w-full max-w-3xl md:max-w-4xl lg:max-w-5xl mx-auto flex flex-col items-center justify-between text-center gap-1 sm:gap-1.5 md:gap-2 my-auto h-full pointer-events-auto">
        ...
        {/* 1. Logo Icon + Headline: NAMMA THANJAI. + Sub-Tagline */}
        <div className="flex flex-col items-center gap-0.5 sm:gap-1 w-full shrink-0 pt-0.5">
          <div className="w-14 h-14 sm:w-18 sm:h-18 md:w-24 md:h-24 rounded-3xl bg-white shadow-lg p-2 sm:p-2.5 flex items-center justify-center shrink-0 border border-slate-200/90 hover:scale-[1.02] transition-transform">
            <img 
          ...
          <p className="text-[10px] sm:text-xs font-bold text-slate-500 max-w-md mx-auto leading-tight uppercase tracking-wider mt-0.5">
            Thanjavur's Local Noticeboard & Helper Directory
          </p>
        </div>
        ...
        <div className="h-6 sm:h-8 flex items-center justify-center overflow-hidden shrink-0 my-0.5">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="bg-slate-900 border border-slate-800 text-yellow-400 font-black text-[11px] sm:text-xs px-3.5 sm:px-4 py-1 rounded-full uppercase tracking-widest text-center shadow-md flex items-center gap-2 select-none"
            >
        ...
        {/* 3. Responsive 3D Mascot Robot Canvas (Balanced frustum FOV 46 distance 4.2: zero top/bottom border cuts) */}
        <div 
          onClick={handleRobotTap} 
          className="w-full max-w-[280px] sm:max-w-[360px] md:max-w-[460px] flex-1 min-h-[170px] sm:min-h-[220px] md:min-h-[260px] max-h-[260px] sm:max-h-[300px] md:max-h-[360px] relative flex items-center justify-center cursor-pointer overflow-visible my-0 bg-transparent"
        >
