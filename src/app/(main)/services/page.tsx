...
export default function ServicesPage() {
  ...
  return (
    <div className="flex flex-col gap-6 mt-6 md:mt-8 pt-2 pb-12">
      ...
      {/* ==========================================
          SAMPLE TECHNICIAN DETAIL MODAL OVERLAY
          ========================================== */}
      {searchParams.get("id") && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/70 backdrop-blur-xs p-0 sm:p-4 animate-fade-in">
          <div className="bg-white border-t-2 sm:border border-slate-200/90 w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl p-6 shadow-2xl relative animate-slide-up sm:animate-scale-up text-slate-800 max-h-[90vh] overflow-y-auto">
            ...
            <button
              onClick={() => router.push(selectedCategory ? `/services?category=${encodeURIComponent(selectedCategory)}` : "/services")}
              ...
              return (
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
                    <div className="w-12 h-12 rounded-2xl bg-amber-500/15 text-amber-700 flex items-center justify-center font-black border border-amber-300">
                      <Wrench className="w-6 h-6 text-amber-600" />
                    </div>
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-wider text-amber-750 bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-200">
                        {sample.trade}
                      </span>
                      <h2 className="font-heading font-black text-base text-slate-900 mt-1">
                        {sample.name}
                      </h2>
                    </div>
                  </div>

                  <div className="flex items-center justify-between text-xs text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-200/60 font-bold">
                    <span>🎖️ {sample.exp}</span>
                    <span>📍 {sample.area}</span>
                  </div>

                  <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Services & Bio</h4>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <a
                      href={`https://wa.me/91${sample.phone}?text=${encodeURIComponent(`Hello ${sample.name}, I found your ${sample.trade} listing on Namma Thanjai. Are you available for a work request in ${sample.area}?`)}`}
                      target="_blank"
