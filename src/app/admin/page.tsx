...
export default function AdminPage() {
  ...
  const handleVerifyPasscode = (e: React.FormEvent) => {
    ...
    if (passcode === "tanjoreadmin") {
      setIsAdmin(true);
      try {
        const confettiObj = require("canvas-confetti");
        confettiObj({ particleCount: 50, spread: 60 });
      } catch (err) {}
    } else {
  ...
  const handleDelete = async (id: string, colName: string) => {
    ...
    try {
      ...
      try {
        const confettiObj = require("canvas-confetti");
        confettiObj({ particleCount: 20, colors: ["#ef4444"] });
      } catch (err) {}
    } catch (error) {
  ...
  const handleToggleVerify = async (item: ModerationItem) => {
    try {
      ...
      if (nextVerify) {
        try {
          const confettiObj = require("canvas-confetti");
          confettiObj({ particleCount: 15, colors: ["#10b981"] });
        } catch (err) {}
      }
    } catch (error) {
  ...
  const handleToggleFeatured = async (item: ModerationItem) => {
    try {
      ...
      if (nextFeatured) {
        try {
          const confettiObj = require("canvas-confetti");
          confettiObj({ particleCount: 20, colors: ["#fbbf24"] });
        } catch (err) {}
      }
    } catch (error) {
  ...
  const handleSeedData = async () => {
    ...
    try {
      ...
      try {
        const confettiObj = require("canvas-confetti");
        confettiObj({ particleCount: 100, spread: 70 });
      } catch (err) {}
      fetchModerationQueue();
    } catch (err: any) {
